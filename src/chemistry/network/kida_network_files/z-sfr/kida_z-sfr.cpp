...
//----------------------------------------------------------------------------------------
//! \fn void ChemNetwork::UpdateRatesSpecial(const Real y[NSCALARS], const Real
//! E) \brief update reaction rates for special reactions with formula = 7 in
//!   z-sfr network
void ChemNetwork::UpdateRatesSpecial(const Real y[NSCALARS], const Real E) {
  ...
  const Real kcr_H_fac =
      2.3 * y_H2 + 1.5 * y_H; // ratio of total to primary rate
  ...
  const Real cHp[7] = {12.25,    8.074e-6, 1.378,   5.087e2,
                       1.586e-2, 0.4723,   1.102e-5};
  const Real cCp[7] = {45.58,    6.089e-3, 1.128,   4.331e2,
                       4.845e-2, 0.8120,   1.333e-4};
  const Real cHep[7] = {5.572,    3.185e-7, 1.512,   5.115e3,
                        3.903e-7, 0.4956,   5.494e-7};
  const Real cSip[7] = {2.166,    5.678e-8, 1.874,   4.375e4,
  ...
  //(15) H + H + gr -> H2 + gr , from Draine book chapter 31.2 page 346, Jura
  //1975
  ...
  if (T > temp_coll) {
    ...
    k10l = 5.996e-30 * std::pow(T, 4.1881) /
           std::pow((1.0 + 6.761e-6 * T), 5.6881) * std::exp(-54657.4 / T);
    ...
    k2body_(id7map_(42)) =
        std::pow(10, std::log10(k10h) * n2ncr / (1. + n2ncr) +
                         std::log10(k10l) / (1. + n2ncr));
    k2body_(id7map_(43)) = std::exp(
        -3.271396786e1 +
        (1.35365560e1 +
         (-5.73932875 +
          (1.56315498 +
           (-2.877056e-1 + (3.48255977e-2 +
                            (-2.63197617e-3 +
                             (1.11954395e-4 + (-2.03914985e-6) * lnTe) * lnTe) *
                                lnTe) *
                               lnTe) *
               lnTe) *
              lnTe) *
  ...
  ksr_(id7map_(21)) =
      1.99e-9 * std::pow(T, -0.190) * fac_H2Oplus_H2 * nH_ * y_O * y_H3plus;
  ksr_(id7map_(22)) =
      1.99e-9 * std::pow(T, -0.190) * fac_H2Oplus_e * nH_ * y_O * y_H3plus;
  ksr_(id7map_(23)) = 1.6e-9 * fac_H2Oplus_H2 * nH_ * y_H2 * y_Oplus;
  ksr_(id7map_(24)) = 1.6e-9 * fac_H2Oplus_e * nH_ * y_H2 * y_Oplus;
  //(28) H2 + C+ + e- -> CHx + H        -- schematic reaction for C+ + H2 ->
  //CH2+ (29) H2 + C+ + e- -> C + H + H      -- schematic reaction for C+ + H2
  //-> CH2+
  ksr_(id7map_(28)) =
      2.31e-13 * std::pow(T, -1.3) * std::exp(-23. / T) * nH_ * y_H2 * y_Cplus;
  ksr_(id7map_(29)) =
      0.99e-13 * std::pow(T, -1.3) * std::exp(-23. / T) * nH_ * y_H2 * y_Cplus;
...
Real CII_rec_rate(const Real temp) {
  ...
  alpharr = A / (term1 * std::pow(1.0 + term1, 1.0 - BN) *
                 std::pow(1.0 + term2, 1.0 + BN));
  alphadr =
      std::pow(temp, -3.0 / 2.0) * (6.346e-9 * std::exp(-1.217e1 / temp) +
                                    9.793e-09 * std::exp(-7.38e1 / temp) +
